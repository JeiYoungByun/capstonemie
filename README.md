...
기존 스마트 미러는 **사용자 부재 시 활용성 부재**, **정적 화면으로 인한 과도한 탐색 비용**, **공용 공간 프라이버시 사고 위험**이 큽니다. 네트워크 지연/장애가 있으면 화면이 비거나 늦게 갱신되고, 방문자 앞에서 개인 일정이 노출되기도 합니다.

### 우리의 해결책: ‘상태’와 ‘이력’ 기반의 **온디바이스 예측** + **역할 기반 공용 정책**

* **예측형 개인화(엣지)**: 등장/시간대/이력으로 **다음 행동 확률**을 계산해 **레이아웃을 동적으로 생성**하고 필요한 데이터를 **미리 불러옴** → 오프라인 30분 시나리오에서도 **p95 반응 ≤ 250ms** 목표.
* **역할 전환형 공용 모드**: Owner/Adult/Kid/Guest와 상황(방문/도어벨)에 따라 **노출 범위/해상도/행동**을 정책적으로 조정(게스트 마스킹, 아동 안전 모드, 공정성 분할/로테이션).
* **세션 메모리 그래프**: 사용자의 과업 맥락을 유지해 **재등장 시 바로 이어서** 볼 수 있게 함.
...
### 2) 가족공용모드 (Roleful Public Mode) - 상세 설계
...
---
이처럼 상태를 세분화하고 각 상태에 맞는 기능을 구체화함으로써, 'Aura'는 단순한 공용 디스플레이를 넘어, 가족 구성원과 공간의 상황을 **스스로 인지하고 오케스트레이션하는 진정한 지능형 허브**로 거듭나게 됩니다. 이 부분이 바로 경쟁 제품들이 따라올 수 없는 우리만의 강력한 UX가 될 것입니다.
---
