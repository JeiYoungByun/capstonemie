...
* **독창성 #1**: 콘텐츠 신선도(클라우드 변화)가 아니라 \*\*사용자 맥락(등장/시간대/최근 이력)\*\*을 트리거로 **예측적 레이아웃/프리페치**를 수행 → *Time-to-Info*·탭 수·주의 전환 **감소**.
* **독창성 #2**: “잠금/슬라이드쇼”가 아닌 **역할(Owner/Adult/Kid/Guest) × 상황(도어벨/방문/시간대)** 기반 **정책 레이어** → **가족 메모/음성**, **합성 캘린더 요약**, **게스트 프라이버시**, **공정성 분할**이 **상태 머신**으로 동작.
* **MVP**: 라즈베리파이5(+NPU 가정), 카메라/마이크, MagicMirror². Python(비전/학습) ↔ Node(모듈/UI) ↔ SQLite(로컬 로그/정책).

---

## Table of Contents

* [Professor Feedback Response Box](#professor-feedback-response-box)
...
## Professor Feedback Response Box

### 1) **클라우드 변화 반응형 vs 온디바이스 패턴 학습 예측형** (독창성 핵심)

* **핵심 차이**: *클라우드형*은 외부 콘텐츠 변화가 **트리거**이고, *Aura*는 **사용자 맥락**(등장/시간대/최근 7일 이력)에서 **다음 행동 확률**을 추정해 **레이아웃을 선재배치**·**데이터 선로딩**.
...
### 2) **가족공용모드 vs 잠금/홀드 화면**

* **핵심 차이**: 잠금/슬라이드쇼는 **정적 UI**, Aura의 **Roleful Public Mode**는 **역할·권한·상황**을 입력으로 **정책 레이어**가 **위젯 단위 접근제어/요약/행동 캡처**를 실행.
